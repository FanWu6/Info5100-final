...
{
   public PieChart_AWT( String title,HashMap<String,Integer> mapdata1) 
   {
      super( title ); 
      CategoryDataset dataset = createDataset(mapdata1);
...
//        chartPanel.setPreferredSize(new java.awt.Dimension(600, 380));
   ...
   private static DefaultCategoryDataset createDataset(HashMap<String,Integer> mapdata1) 
   {
...
//            System.out.println(entry.getKey()+"("+entry.getValue()+"/"+(total*100)+"%"+")");
            dataset.addValue( entry.getValue(), entry.getKey()+"("+(Math.round(entry.getValue()/total*100))+"%"+")", "Agency proformance");
       }
//       System.out.println("---------");
       total = 0.0f;  
      return dataset;         
